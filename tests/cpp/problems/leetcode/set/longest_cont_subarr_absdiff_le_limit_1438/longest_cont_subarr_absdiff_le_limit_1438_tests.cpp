...
using namespace algo::problems::leetcode::set::longest_cont_subarr_absdiff_le_limit;
using namespace algo::cpp::ds::tests::utils;

class LongestSubarrayUsingSetsTests : public ::testing::Test {
protected:
    void SetUp() override {}
...
TEST_F(LongestSubarrayUsingSetsTests, LeetCodeSample1) {
    runTest({8,2,4,7}, 4, 2);
}

TEST_F(LongestSubarrayUsingSetsTests, LeetCodeSample2) {
    runTest({10,1,2,4,7,2}, 5, 4);
}

TEST_F(LongestSubarrayUsingSetsTests, LeetCodeSample3) {
    runTest({4,2,2,2,4,4,2,2}, 0, 3);
}


TEST_F(LongestSubarrayUsingSetsTests, RunTestsFromResource) {
    using namespace algo::cpp::ds::tests::utils::leetcode;
    auto path = get_test_ds_dir("set");
