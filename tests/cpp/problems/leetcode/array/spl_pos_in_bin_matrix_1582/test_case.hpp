...
public:
    ...
    [[nodiscard]] std::vector<std::vector<int>> getInput() {
        return input;
    }
...
private:
    ...
    static std::vector<std::vector<int>> loadMatrix(boost::json::array json_array) {
        std::vector<std::vector<int>> result;
        for (auto &element: json_array) {
            result.emplace_back(getVector(element.as_array()));
        }

        return result;
    }

    static std::vector<int> getVector(boost::json::array json_array) {
